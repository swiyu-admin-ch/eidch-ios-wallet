...
final class IdentityTrustStatementV1Tests: XCTestCase {
  ...
  func testDecode_allFields() throws {
    let trustStatement = IdentityTrustStatementV1JWT.Mock.allFields

    let payload = trustStatement.payload

    let expectedStatusList = VcSdJwtTokenStatus(statusList: VcSdJwtTokenStatus.StatusList(index: 111, uri: "status_list_uri"))
  ...
  func testGetLocalizedEntityName_multipleLanguages_returnsFirstValidLanguage() {
    let trustStatement = IdentityTrustStatementV1JWT.Mock.validSample
