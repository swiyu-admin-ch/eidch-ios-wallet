...
final class OcaBundleTests: XCTestCase {
  ...
  func testDecode_dataSource1x0SimpleSample() throws {
    let ocaBundle = OcaBundle.Mock.dataSource1x0SimpleSample

    XCTAssertEqual(ocaBundle.captureBases.count, 1)
    XCTAssertEqual(ocaBundle.overlays.count, 1)

    let dataSourceOverlay = try XCTUnwrap(ocaBundle.overlays.first as? DataSourceOverlay1x0)
    XCTAssertEqual(dataSourceOverlay.captureBaseDigest, "IL00eAbH9tHLBN0s6qIZyVVmm6vYA3wsakyqnFMU1nL4")
    XCTAssertEqual(dataSourceOverlay.format, "vc+sd-jwt")
    XCTAssertEqual(dataSourceOverlay.attributeSources["key"], [.string("firstnames"), .index(1)])
  }
  ...
  func testGetLatestOverlaysOfType_oldVersionOnly_returnsOldOverlay() throws {
    let captureBase = CaptureBase1x0(digest: "digest", attributes: ["attribute": .text], classification: nil, flaggedAttributes: nil)
    let overlay = DataSourceOverlay1x0(captureBaseDigest: "digest", format: "vc+sd-jwt", attributeSources: ["attribute": [.string("claim")]])
    let ocaBundle = try OcaBundle(captureBases: [captureBase], overlays: [overlay])

    let dataSourceOverlays = ocaBundle.getLatestOverlaysOfType(overlayType: .dataSource, digest: "digest")

    XCTAssertEqual(dataSourceOverlays.count, 1)
    XCTAssertEqual(dataSourceOverlays[0] as? DataSourceOverlay1x0, overlay)
  }

  func testGetLatestOverlaysOfType_twoVersions_returnsLatestOverlay() throws {
    let captureBase = CaptureBase1x0(digest: "digest", attributes: ["attribute": .text], classification: nil, flaggedAttributes: nil)
    let oldOverlay = DataSourceOverlay1x0(captureBaseDigest: "digest", format: "vc+sd-jwt", attributeSources: ["attribute": [.string("claim")]])
    let overlay = DataSourceOverlay2x0(captureBaseDigest: "digest", format: "dc+sd-jwt", attributeSources: ["attribute": [.string("claim")]])
    let ocaBundle = try OcaBundle(captureBases: [captureBase], overlays: [oldOverlay, overlay])

    let dataSourceOverlays = ocaBundle.getLatestOverlaysOfType(overlayType: .dataSource, digest: "digest")

    XCTAssertEqual(dataSourceOverlays.count, 1)
    XCTAssertEqual(dataSourceOverlays[0] as? DataSourceOverlay2x0, overlay)
  }
