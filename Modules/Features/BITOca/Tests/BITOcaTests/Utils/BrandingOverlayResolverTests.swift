...
final class BrandingOverlayResolverTests: XCTestCase {
  ...
  func testResolve_attributesInDifferentDataSources_resolves() throws {
    let dataSourceOverlayA = try Self.createDataSourceOverlay(digest: "A", attributeSources: ["firstname": JsonPath(rawString: "$.firstname")])
    let dataSourceOverlayB = try Self.createDataSourceOverlay(digest: "B", attributeSources: ["lastname": JsonPath(rawString: "$.lastname")])
  ...
  private static let attributeSources: [String: JsonPath] = try! [
    "firstname": JsonPath(rawString: "$.firstname"),
    "lastname": JsonPath(rawString: "$.lastname"),
    "city": JsonPath(rawString: "$.city"),
    "country": JsonPath(rawString: "$.country"),
  ]
  ...
  private static func createDataSourceOverlay(digest: String = digestMock, format: String = formatMock, attributeSources: [AttributeKey: JsonPath] = attributeSources) -> DataSourceOverlay1x0 {
    DataSourceOverlay1x0(captureBaseDigest: digest, format: formatMock, attributeSources: attributeSources)
  }
