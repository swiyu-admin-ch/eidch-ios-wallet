...
final class WalletPairingOfferViewModelTests: XCTestCase {
  ...
  func testFetchPairingQRCode_WhenCaseIdExists_ShouldFetchSuccessfully() async throws {
  ...
  func testRetryFetching_ShouldCallFetchPairingQRCode() async throws {
    let expectedCaseId = "test-case-id"
    context.caseId = expectedCaseId
  ...
  func testSetState_ShouldUpdateStateWithAnimation() async throws {
    let expectedCaseId = "test-case-id"
    let expectedQRCodeData = Data("test-qr-code".utf8)
